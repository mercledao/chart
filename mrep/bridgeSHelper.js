const { formulas, getInputData, writeOutput, getVolWeight, getDaysInBtw } = require('./utils');
...
const getAngleInRadians = (src, dest) => {
    ...
    const angle = (parseFloat(4 * matrix[src][dest] + 1) * Math.PI) / parseFloat(2 * delta);
    return angle;
};
...
const bridgeS = (txns) => {
    ...
    for (let i = 0; i < txns.length; i++) {
        if (!txns[i].token_bought_symbol?.length || !txns[i].token_sold_symbol?.length) continue;
